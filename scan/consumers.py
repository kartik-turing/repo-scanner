...
class ScanConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # Pass the WebSocket connection (self) to RepoScanner
        self.scanner = RepoScanner(self.scan_id, websocket=self)

        # Log the connection attempt
        logger.info(f"WebSocket connection attempt for scan {self.scan_id}")
    ...
    async def run_scan(self, scan_id, repo_url):
        ...
        try:
            ...
            # Notify the client that the scan is starting
            await self.send(
                text_data=json.dumps(
                    {
                        "message": "Retrieved repository files",
                        "scan_id": self.scan_id,
